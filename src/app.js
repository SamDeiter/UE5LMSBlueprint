...
class BlueprintApp {
  ...
  static init() {
    ...
    BlueprintApp.localVariablesController = new LocalVariablesController(
      BlueprintApp
    );
    ...
    window.setTask = (taskId) =>
      BlueprintApp.taskManager.setCurrentTask(taskId);
    ...
    BlueprintApp.switchGraph = (graphName) =>
      BlueprintApp.graphSwitcher.switchGraph(graphName);
    ...
    if (saveBtn)
      saveBtn.addEventListener("click", () =>
        BlueprintApp.persistence.save(true)
      );
    ...
    if (stepBtn)
      stepBtn.addEventListener("click", () => BlueprintApp.sim.stepOver());

    const stepIntoBtn = document.getElementById(DOMElements.STEP_INTO_BTN);
    if (stepIntoBtn)
      stepIntoBtn.addEventListener("click", () => BlueprintApp.sim.stepInto());

    const stepOutBtn = document.getElementById(DOMElements.STEP_OUT_BTN);
    if (stepOutBtn)
      stepOutBtn.addEventListener("click", () => BlueprintApp.sim.stepOut());
    ...
    const newBlueprintMenuItem = document.getElementById(
      "new-blueprint-menu-item"
    );
    ...
    if (classSettingsBtn) {
      classSettingsBtn.addEventListener("click", () => {
        if (BlueprintApp.details) {
          BlueprintApp.details.showClassSettings();
        }
      });
    }
    ...
    document.addEventListener("keydown", (e) => {
      ...
      if (e.key === "Delete" || e.key === "Backspace") {
        ...
        if (BlueprintApp.graph && BlueprintApp.graph.selectedNodes.size > 0)
          return;
        if (BlueprintApp.wiring && BlueprintApp.wiring.selectedLinks.size > 0)
          return;
        ...
        const hasSelectedComponents =
          BlueprintApp.componentsController &&
          BlueprintApp.componentsController.selectedComponentIds.size > 0;
        ...
        if (!varToDelete && !hasSelectedComponents) {
          const activeEl = document.activeElement;
          if (activeEl) {
            const focusedCompEl = activeEl.closest(
