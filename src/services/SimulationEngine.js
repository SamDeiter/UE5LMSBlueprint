...
export class SimulationEngine {
  ...
  run() {
    ...
    if (this.app.compiler.isDirty) {
      this.log(
        "Simulation halted: Compile failed. Fix errors before playing.",
        "error"
      );
      return;
    }
  ...
  updateUI() {
    ...
    if (this.playBtn) {
      ...
      this.playBtn.disabled =
        (this.isRunning && !this.isPaused) || needsCompile;
  ...
  startTickLoop() {
    ...
    const tick = (timestamp) => {
      ...
      const tickNodes = [...this.app.graph.nodes.values()].filter(
      ...
      this.timelines.forEach((state, nodeId) => {
        ...
        const alpha = Math.max(
          0,
          Math.min(1, state.currentTime / state.length)
        );
  ...
  async executeFlow(startNode, startPinId = null) {
    ...
    while (currentNode && this.isRunning && steps < maxSteps) {
      ...
      const nextPinId = await this.executeNodeLogic(
        currentNode,
        currentInputPin
      );
  ...
  evaluatePin(pin) {
    ...
    // First check pin.links, then fallback to searching Wiring Map directly
    let linkId = null;
    let link = null;

    if (pin.isConnected()) {
      linkId = pin.links[0]; // Data inputs only have one link
      link = this.app.wiring.links.get(linkId);
    } else {
      // Fallback: search Wiring Map directly for connections to this pin
      // This handles cases where pin.links wasn't properly synced during load
      for (const [id, l] of this.app.wiring.links) {
        if (l.endPin.id === pin.id) {
          link = l;
          linkId = id;
          // Sync the pin.links array for future lookups
          if (!pin.links.includes(id)) {
            pin.links.push(id);
          }
          break;
        }
  ...
  evaluateNeedNodes() {
    ...
    needNodes.forEach((node) => {
      ...
      if (!needData || !needData.criteria) {
        this.log(
          `NeedNode "${node.title}" has no criteria configured.`,
          "error"
        );
        return;
      }
      ...
      this.log(
        `  Score: ${nodeScore}% (Threshold: ${needData.passThreshold}%)`
      );
      this.log(
    ...
    const overallScore =
      totalWeight > 0 ? Math.round(totalScore / needNodes.length) : 0;
  ...
  reportToSCORM(score, passed) {
    ...
    if (!scormClient.isInitialized) {
      ...
      if (!initialized) {
        ...
        this.log(
          `[SCORM] Would report: Score = ${score}%, Status = ${
    ...
    this.log(
      `\n[SCORM] ✅ Reported to LMS: Score = ${score}%, Status = ${
        passed ? "passed" : "failed"
      } `,
      "success"
    );
