...
export class HistoryManager {
  ...
  saveState(actionType = "action") {
    ...
    const variablesArray =
      this.app.variables && this.app.variables.variables
        ? [...this.app.variables.variables.values()]
        : [];
    const functionsArray = this.app.functionRegistry
      ? this.app.functionRegistry.getAll().map((f) => f.toJSON())
      : [];
    const macrosArray = this.app.macroRegistry
      ? this.app.macroRegistry.getAll().map((m) => m.toJSON())
      : [];
    const componentsArray = this.app.components
      ? [...this.app.components.values()]
      : [];
    ...
    if (
      ...
      this.undoStack[this.undoStack.length - 1].state === stateJSON
    ) {
      return;
    }
  ...
  applyState(stateJSON) {
    ...
    try {
      ...
      if (this.app.wiring && this.app.wiring.links)
      ...
      // Clear DOM elements
      if (this.app.graph && this.app.graph.nodesContainer)
      ...
      const activeGraphData = this.app.graphs[this.app.activeGraph] || {
        nodes: [],
        links: [],
      };
      ...
      // Notify StateManager subscribers of all changes
      if (this.app.stateManager) {
        this.app.stateManager.notifyAll();
      }
    } catch (e) {
      this.app.compiler.log(
        `Error performing history action: ${e.message}`,
        "error"
      );
  ...
  undo() {
    ...
    if (prevState) {
      ...
      this.app.compiler.log(
        `Undo successful: Rolled back one ${prevState.action}.`
      );
    }
