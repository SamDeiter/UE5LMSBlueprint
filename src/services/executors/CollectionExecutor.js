...
export class CollectionExecutor extends BaseExecutor {
  async execute(node) {
    ...
    switch (node.nodeKey) {
      // ============ ARRAY ============
      case "Array_Add": {
        const arr = this.evaluateInput(node, "array") || [];
        const item = this.evaluateInput(node, "item");
        arr.push(item);
      ...
      case "Array_RemoveIndex": {
        const arr = this.evaluateInput(node, "array") || [];
        const index = parseInt(this.evaluateInput(node, "index")) || 0;
      ...
      // ============ SET ============
      case "Set_Add": {
        const set = this.evaluateInput(node, "set") || new Set();
        const item = this.evaluateInput(node, "item");
      ...
      // ============ MAP ============
      case "Map_Add": {
        const map = this.evaluateInput(node, "map") || new Map();
        const key = this.evaluateInput(node, "key");
        const value = this.evaluateInput(node, "value");
