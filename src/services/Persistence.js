...
export class Persistence {
  ...
  save(forceCapture = false) {
    try {
      ...
      const stateJSON =
        this.app.history.undoStack[this.app.history.undoStack.length - 1].state;
      ...
      this.app.compiler.log(
        `Failed to save graph. Error: ${e.message}.`,
        "error"
      );
    }
  }
  ...
  load() {
    try {
      ...
      if (stateJSON) {
        ...
        this.app.history.undoStack.push({
          state: finalStateJSON,
          action: "initial load",
        });
  ...
  serializeNodes() {
    return this.app.graph && this.app.graph.nodes
      ? [...this.app.graph.nodes.values()].map((node) => ({
          id: node.id,
          title: node.title,
          x: node.x,
          y: node.y,
  ...
  serializeLinks() {
    return this.app.wiring && this.app.wiring.links
      ? [...this.app.wiring.links.values()].map((link) => ({
          id: link.id,
  ...
  loadDefaultGraph() {
    ...
    if (this.app.graph && this.app.history) {
      ...
      console.error(
        "Persistence.loadDefaultGraph: Required components (graph/history) are undefined."
      );
