...
export const registerRefactorTests = (runner) => {
  runner.register("Constants Check", (_app) => {
    if (PIN_TYPES.EXEC !== "exec")
      throw new Error('PIN_TYPES.EXEC should be "exec"');
    if (PIN_COLORS.exec !== "var(--color-exec)")
      throw new Error('PIN_COLORS.exec should be "var(--color-exec)"');
  });

  runner.register("Utils Constant Mapping", (_app) => {
    const color = Utils.getPinColor("exec");
    if (color !== "var(--color-exec)")
      throw new Error(`Utils.getPinColor('exec') returned ${color}`);

    const typeClass = Utils.getPinTypeClass("exec");
    if (typeClass !== "exec-pin")
      throw new Error(`Utils.getPinTypeClass('exec') returned ${typeClass}`);
  });
  ...
  runner.register("Node Rendering Sanity", (app) => {
    ...
    if (element.tagName !== "DIV")
      throw new Error("Node.render() should return a DIV");
    if (!element.classList.contains("pure-node-node"))
      throw new Error('Node element missing "pure-node-node" class');
  });
};
