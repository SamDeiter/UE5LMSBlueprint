...
export class ParentClassModal {
    ...
    init() {
        ...
        this.modal.className = 'modal-overlay'; // Reusing existing modal overlay class if available, or define new
            ...
            <div class="modal-content parent-class-content">
                ...
                <div class="common-classes-section">
                    ...
                    <div class="class-buttons-grid">
                        ${this.createClassButton('Actor', 'An Actor is an object that can be placed or spawned in the world.', 'fas fa-cube')}
                        ${this.createClassButton('Pawn', "A Pawn is an actor that can be 'possessed' and receive input from a controller.", 'fas fa-chess-pawn')}
                        ${this.createClassButton('Character', "A character is a type of Pawn that includes the ability to walk around.", 'fas fa-user')}
                        ${this.createClassButton('PlayerController', "A Player Controller is an actor responsible for controlling a Pawn used by the player.", 'fas fa-gamepad')}
                        ${this.createClassButton('GameModeBase', "Game Mode Base defines the game being played, its rules, scoring, and other facets of the game type.", 'fas fa-cogs')}
                        ${this.createClassButton('ActorComponent', "An ActorComponent is a reusable component that can be added to any actor.", 'fas fa-puzzle-piece')}
                        ${this.createClassButton('SceneComponent', "A Scene Component is a component that has a scene transform and can be attached to other scene components.", 'fas fa-project-diagram')}
                    </div>
                </div>

                <div class="all-classes-section">
                    <div class="section-title toggle-all-classes"><i class="fas fa-caret-right"></i> ALL CLASSES</div>
                    <div class="all-classes-content" style="display:none;">
        ...
        // Event Listeners
        this.modal.querySelector('#parent-class-close').addEventListener('click', () => this.close());
        this.modal.querySelector('#parent-class-cancel').addEventListener('click', () => this.close());
