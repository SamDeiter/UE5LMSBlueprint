...
export class FunctionExecutor extends BaseExecutor {
  ...
  async executeFunctionCall(node) {
    ...
    await this.engine.flow.executeFlow(entryNode);
  ...
  async executeCustomEventCall(node) {
    ...
    const targetNode = [...this.app.graph.nodes.values()].find(
      (n) => n.nodeKey === "CustomEvent" && n.title === eventName
    );

    if (!targetNode) {
      this.log(
        `Error: Custom Event '${eventName}' not found in this graph.`,
        "error"
      );
      return null;
    }
    ...
    await this.engine.flow.executeFlow(targetNode);
